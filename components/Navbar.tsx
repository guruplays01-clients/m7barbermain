      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="text-sm font-medium hover:text-[#D4AF37] transition-colors duration-200"
              >
                {link.name}
              </a>
            ))}
            <button 
              ...
              className="flex items-center space-x-2 border border-[#D4AF37] text-[#D4AF37] px-4 py-2 rounded hover:bg-[#D4AF37] hover:text-black transition-all duration-300 group"
            >
            ...
            <a
              href="#contact"
              className="gold-bg text-black px-6 py-2 rounded font-bold hover:scale-105 transition-transform duration-200"
            >
              BOOK NOW
            </a>
          </div>
          ...
          <div className="md:hidden flex items-center space-x-4">
            <button 
              onClick={onOpenAdvisor}
              className="p-2 border border-[#D4AF37] text-[#D4AF37] rounded-full"
            >
        ...
        <div className="px-4 pt-2 pb-6 space-y-2">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className="block px-3 py-4 text-lg font-medium border-b border-neutral-900"
          ...
          <div className="pt-4">
             <a
              href="#contact"
              className="block w-full text-center gold-bg text-black py-4 rounded font-bold"
              onClick={() => setIsOpen(false)}
            >
              BOOK APPOINTMENT
            </a>
